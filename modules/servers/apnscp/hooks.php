...
add_hook('ClientAreaPrimarySidebar', 1, function($sidebar) {
	...
	$sidebar->getChild("Service Details Actions")->addChild("Login to ApisCP", ["uri" => "clientarea.php?action=productdetails&id=".$service->id."&dosinglesignon=1", "label" => 'Login to ApisCP', "attributes" => ["target" => "_blank"], "disabled" => $service->status != "Active", "order" => 1]);
});
